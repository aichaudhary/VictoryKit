...
const ContainerScanService = require('../services/biometricService');
...
class BiometricAuthMiddleware {
  ...
  async authenticateBiometric(req, res, next) {
    try {
      ...
      const result = await ContainerScanService.authenticate(userId, biometricData, {
      ...
      if (!result.success) {
        ...
        await ContainerScanService.logSecurityEvent({
  ...
  async securityMonitor(req, res, next) {
    ...
    res.send = function(data) {
      ...
      if (req.path.includes('/auth') || req.path.includes('/biometric')) {
        ContainerScanService.logSecurityEvent({
          type: 'API_ACCESS',
          method: req.method,
          path: req.path,
          ip: req.ip,
