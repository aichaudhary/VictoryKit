...
// API Routes
app.use('/api/v1/incidentcommand', routes);
...
// Database connection
const MONGODB_URI =
  process.env.MONGODB_URI || 'mongodb://localhost:27017/victorykit_incidentcommand';

mongoose
  .connect(MONGODB_URI)
  .then(() => {
    console.log('✅ Connected to MongoDB - victorykit_incidentcommand');

    // Initialize WebSocket service
    websocketService.initialize(server);

    server.listen(PORT, () => {
      console.log(`🚨 incidentcommand API running on port ${PORT}`);
