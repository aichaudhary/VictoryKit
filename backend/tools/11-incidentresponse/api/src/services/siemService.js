...
class SIEMService {
  constructor() {
    // Splunk
    this.splunkToken = process.env.incidentcommand_SPLUNK_API_TOKEN;
    this.splunkBaseUrl = process.env.incidentcommand_SPLUNK_BASE_URL;
    this.splunkIndex = process.env.incidentcommand_SPLUNK_INDEX || 'security_incidents';

    // Elastic SIEM
    this.elasticApiKey = process.env.incidentcommand_ELASTIC_API_KEY;
    this.elasticBaseUrl = process.env.incidentcommand_ELASTIC_BASE_URL;
    this.elasticIndex = process.env.incidentcommand_ELASTIC_INDEX || 'incidents';

    // Azure Sentinel
    this.sentinelWorkspaceId = process.env.incidentcommand_AZURE_SENTINEL_WORKSPACE_ID;
    this.sentinelClientId = process.env.incidentcommand_AZURE_SENTINEL_CLIENT_ID;
    this.sentinelClientSecret = process.env.incidentcommand_AZURE_SENTINEL_CLIENT_SECRET;
    this.sentinelTenantId = process.env.incidentcommand_AZURE_SENTINEL_TENANT_ID;
  ...
  async searchSentinel(query, timeRange) {
    try {
      ...
      const logs =
        response.data.tables[0]?.rows.map((row) => {
          const columns = response.data.tables[0].columns;
          const logObj = {};
          columns.forEach((col, idx) => {
            logObj[col.name] = row[idx];
          });
          return {
