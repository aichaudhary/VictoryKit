# 🎯 BehaviorAnalytics - AI-Powered Enterprise Risk Assessment Engine
...
**MongoDB Database**: `behavioranalytics_db`
...
cd backend/tools/29-behavioranalytics/api
npm install
...
**Backend API** (`backend/tools/29-behavioranalytics/api/.env`):
```env
PORT=4029
MONGODB_URI=mongodb://mongodb:27017/behavioranalytics_db
NODE_ENV=production
```

**AI Assistant** (`backend/tools/29-behavioranalytics/ai-assistant/.env`):
```env
AI_PORT=6029
...
**Frontend** (`frontend/tools/29-behavioranalytics/.env`):
```env
...
cd backend/tools/29-behavioranalytics/ai-assistant
npm start
...
ws.onopen = () => {
  console.log('Connected to BehaviorAnalytics Assistant');
};
...
**BehaviorAnalytics** - *Quantify Risk. Prioritize Action. Reduce Impact.*
