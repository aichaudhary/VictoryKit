...
// Middleware
app.use(cors({
  origin: ['http://localhost:3029', 'http://behavioranalytics.maula.ai', 'https://behavioranalytics.maula.ai'],
  credentials: true
}));
...
// MongoDB Connection
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://mongodb:27017/behavioranalytics_db';
...
mongoose.connect(MONGODB_URI, {
  ...
  .then(() => {
    console.log('✅ Connected to MongoDB (behavioranalytics_db)');
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 BehaviorAnalytics API Server running on port ${PORT}`);
