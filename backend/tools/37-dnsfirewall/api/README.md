...
#### Analyze DNS Query
```http
POST /api/v1/dnsfirewall/analyze
...
```http
POST /api/v1/dnsfirewall/analyze/batch
Authorization: Bearer <token>
Content-Type: application/json
...
#### Create DNS Policy
```http
POST /api/v1/dnsfirewall/policies
...
#### Get DNS Policies
```http
GET /api/v1/dnsfirewall/policies?page=1&limit=20&type=domain_filter&enabled=true
Authorization: Bearer <token>
```
...
#### Get DNS Query Logs
```http
GET /api/v1/dnsfirewall/logs?page=1&limit=50&startDate=2024-01-01&endDate=2024-01-31&threatLevel=high
Authorization: Bearer <admin-token>
```

#### Get Real-time DNS Statistics
```http
GET /api/v1/dnsfirewall/stats/realtime?timeRange=24h&includeThreats=true
Authorization: Bearer <admin-token>
```

#### Get Domain Reputation
```http
GET /api/v1/dnsfirewall/reputation/{domain}
Authorization: Bearer <token>
```
...
```http
GET /api/v1/dnsfirewall/threats/feeds?source=virustotal&limit=100
Authorization: Bearer <admin-token>
```

#### Update Threat Intelligence
```http
POST /api/v1/dnsfirewall/threats/update
Authorization: Bearer <admin-token>
Content-Type: application/json
...
#### Generate DNS Report
```http
POST /api/v1/dnsfirewall/reports/generate
Authorization: Bearer <admin-token>
Content-Type: application/json
...
#### Get DNS Analytics
```http
GET /api/v1/dnsfirewall/analytics?metric=queries_per_hour&timeRange=7d
Authorization: Bearer <admin-token>
```
...
#### Authentication
```env
DNSSHIELD_JWT_SECRET=your-dnsfirewall-jwt-secret-key
JWT_EXPIRES_IN=24h
```
...
const DNSShieldClient = {
  async analyzeDomain(domain, token) {
    const response = await fetch('/api/v1/dnsfirewall/analyze', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
  ...
  async getPolicies(token) {
    const response = await fetch('/api/v1/dnsfirewall/policies', {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
...
class DNSShieldClient:
    ...
    def analyze_domain(self, domain):
        response = requests.post(
            f"{self.base_url}/api/v1/dnsfirewall/analyze",
            json={"domain": domain},
    ...
    def get_realtime_stats(self):
        response = requests.get(
            f"{self.base_url}/api/v1/dnsfirewall/stats/realtime",
            headers=self.headers
        )
        return response.json()
