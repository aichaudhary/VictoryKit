/**
 * RiskQuantify API Server - Enhanced Edition
...
const logger = winston.createLogger({
  ...
  defaultMeta: { service: "riskquantify-api" },
  transports: [
...
const connectDB = async (retries = 5) => {
  const MONGODB_URI = process.env.MONGODB_URI || "mongodb://localhost:27017/victorykit_riskquantify";
...
connectDB().then(() => {
  server.listen(PORT, () => {
    logger.info(`RiskQuantify API running on port ${PORT}`);
