/**
 * RiskQuantify AI Assistant Tests
 * Tool 19 - AI Assistant Testing Suite
 */

import { WebSocket } from 'ws';
import { RiskQuantifyAIAssistant } from './server';
import { RiskQuery, AnalysisType, AIResponse } from './types';

describe('RiskQuantify AI Assistant', () => {
  let server: RiskQuantifyAIAssistant;
  const PORT = 6019;

  beforeAll(async () => {
    server = new RiskQuantifyAIAssistant();
  ...
  describe('WebSocket Connection', () => {
    ...
    test('should receive welcome message on connection', (done) => {
      ...
      ws.on('message', (data: Buffer) => {
        ...
        expect(message.content).toContain('RiskQuantify AI Assistant');
        ws.close();
        done();
      });
