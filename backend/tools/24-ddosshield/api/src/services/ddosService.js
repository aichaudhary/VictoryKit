...
const ddosService = {
    ...
    async detectAttack(trafficData) {
        ...
        if (trafficData.metrics?.bandwidth?.inbound) {
            const bwRatio = trafficData.metrics.bandwidth.inbound / (baseline.bandwidth || 100);
            if (bwRatio > 5) {
                signals.push({ signal: 'extreme_bandwidth', weight: 40, value: bwRatio });
                score += 40;
            } else if (bwRatio > 3) {
        ...
        if (trafficData.metrics?.packets?.inbound) {
            const pktRatio = trafficData.metrics.packets.inbound / (baseline.packets || 1000);
            if (pktRatio > 5) {
                signals.push({ signal: 'extreme_packet_rate', weight: 35, value: pktRatio });
                score += 35;
            } else if (pktRatio > 3) {
                signals.push({ signal: 'high_packet_rate', weight: 20, value: pktRatio });
                score += 20;
            }
        }
        ...
        if (trafficData.metrics?.requests?.rate) {
            const reqRatio = trafficData.metrics.requests.rate / (baseline.requests || 100);
            if (reqRatio > 10) {
                signals.push({ signal: 'extreme_request_rate', weight: 30, value: reqRatio });
                score += 30;
            }
        }
        ...
        if (trafficData.source?.totalIps > 1000) {
            signals.push({ signal: 'distributed_attack', weight: 15, value: trafficData.source.totalIps });
            score += 15;
        }
    ...
    async analyzeTraffic(trafficData) {
        ...
        if (metrics.bandwidth?.inbound > baseline.bandwidth * 2) {
            signals.push({ signal: 'bandwidth_anomaly', value: metrics.bandwidth.inbound });
            anomalyScore += 25;
        }
    ...
    async checkForAnomalies(records) {
        ...
        // Calculate average
        const avgBandwidth = previous.reduce((sum, r) => 
            sum + (r.metrics?.bandwidth?.inbound || 0), 0) / previous.length;
    ...
    getDefaultMitigationActions(attack) {
        ...
        if (attack.source?.topIps?.length > 0) {
            actions.push({ type: 'block_ips', target: attack.source.topIps.slice(0, 100) });
        }
    ...
    async applyMitigationAction(action, attack) {
        try {
            switch (action.type) {
                case 'block_ips':
                    const ips = Array.isArray(action.target) ? action.target : [action.target];
                    for (const ip of ips) {
    ...
    classifyAttack(trafficData, signals) {
        ...
        if (signalNames.includes('extreme_bandwidth') || signalNames.includes('high_bandwidth')) {
