...
// System prompt for KubeArmor AI
const SYSTEM_PROMPT = `You are KubeArmor AI, an expert in enterprise password and secrets management with deep knowledge of cryptography, secure storage, access control, and credential lifecycle management.
...
You have access to functions that allow you to interact with the KubeArmor system. Use them to provide real-time assistance.`;

// Create WebSocket server
const wss = new WebSocket.Server({ port: PORT, path: '/maula/ai' });

console.log(`KubeArmor AI Assistant WebSocket server running on port ${PORT}`);

wss.on('connection', (ws) => {
  console.log('New client connected to KubeArmor AI');
  ...
  ws.on('close', () => {
    console.log('Client disconnected from KubeArmor AI');
  });
});
