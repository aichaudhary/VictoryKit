# KubeArmor - AI-Powered Enterprise Password & Secrets Management
...
- **Frontend**: `https://kubearmor.maula.ai` (Production) | `http://localhost:3033` (Development)
...
KubeArmor is an enterprise-grade password and secrets management solution that provides:
...
# Backend API
cd backend/tools/33-kubearmor/api
npm install
cp .env.example .env
npm start
...
# Frontend
cd ../../../../frontend/tools/33-kubearmor
npm install
npm run dev
```
...
# API (.env)
PORT=4033
MONGODB_URI=mongodb://localhost:27017/kubearmor_db
...
See `kubearmor-config.json` for full configuration:

```json
{
  "toolName": "KubeArmor",
  "subdomain": "kubearmor.maula.ai",
  "port": 3033,
  "apiPort": 4033,
  "aiPort": 6033,
  "theme": {
