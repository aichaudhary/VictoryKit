...
wss.on('connection', (ws: WebSocket) => {
  ...
  console.log(`RiskAssess AI Assistant: New connection ${connectionId}`);

  // Send welcome message
  const welcomeMessage: AIResponse = {
    id: uuidv4(),
    type: 'welcome',
    content: 'Welcome to RiskAssess AI Assistant. I can help you with risk analysis, threat modeling, impact assessment, compliance evaluation, and mitigation strategies. How can I assist with your risk management today?',
    timestamp: new Date()
  };
  ...
  ws.on('close', () => {
    ...
    console.log(`RiskAssess AI Assistant: Connection ${connectionId} closed`);
  });

  ws.on('error', (error) => {
    console.error(`RiskAssess AI Assistant: Connection ${connectionId} error:`, error);
    connections.delete(connectionId);
  });
});
...
Identify compliance gaps and recommend remediation actions.`;
  ...
  try {
    ...
    return fallbackComplianceCheck(data);
  }
}
...
function fallbackComplianceCheck(data: any): AIResponse {
  return {
    id: uuidv4(),
    type: 'compliance_check',
...
console.log(`RiskAssess AI Assistant server running on port ${PORT}`);
