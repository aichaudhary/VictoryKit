...
const model = genAI.getGenerativeModel({ 
  ...
  systemInstruction: `You are IoTSecure AI, an expert in IoT security with deep knowledge of:
...
wss.on('connection', (ws, req) => {
  ...
  console.log(`[IoTSecure AI] Client connected: ${clientId}`);
  ...
  ws.on('message', async (data) => {
    try {
      ...
      console.error('[IoTSecure AI] Error handling message:', error);
      ws.send(JSON.stringify({
        type: 'error',
  ...
  ws.on('close', () => {
    console.log(`[IoTSecure AI] Client disconnected: ${clientId}`);
  ...
  ws.on('error', (error) => {
    console.error(`[IoTSecure AI] WebSocket error for ${clientId}:`, error);
  });
});
...
async function handleChatMessage(ws, session, payload) {
  ...
  try {
    ...
    console.error('[IoTSecure AI] Chat error:', error);
    ws.send(JSON.stringify({
      type: 'error',
...
async function handleFunctionCall(ws, session, payload) {
  ...
  try {
    ...
    console.error('[IoTSecure AI] Function call error:', error);
    ws.send(JSON.stringify({
      type: 'error',
...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: 'IoTSecure AI Assistant',
    version: '1.0.0',
    model: 'gemini-1.5-pro',
