/**
 * WirelessHunter AI Assistant Server
...
const model = genAI.getGenerativeModel({ 
  ...
  systemInstruction: `You are WirelessHunter AI, an expert wireless network security analyst specializing in:
...
wss.on('connection', (ws, req) => {
  ...
  console.log(`[WirelessHunter AI] Client connected: ${clientId}`);
  ...
  ws.on('message', async (data) => {
    try {
      ...
      console.error('[WirelessHunter AI] Error handling message:', error);
      ws.send(JSON.stringify({
        type: 'error',
  ...
  ws.on('close', () => {
    console.log(`[WirelessHunter AI] Client disconnected: ${clientId}`);
  ...
  ws.on('error', (error) => {
    console.error(`[WirelessHunter AI] WebSocket error for ${clientId}:`, error);
  });
});
...
async function handleMessage(ws, clientId, message) {
  ...
  switch (type) {
    ...
    case 'context_update':
      ...
      break;
    default:
      console.warn(`[WirelessHunter AI] Unknown message type: ${type}`);
  }
}
...
async function handleChatMessage(ws, session, payload) {
  ...
  try {
    ...
    console.error('[WirelessHunter AI] Chat error:', error);
    ws.send(JSON.stringify({
      type: 'error',
      payload: {
        messageId,
...
async function handleFunctionCall(ws, session, payload) {
  ...
  try {
    ...
    console.error('[WirelessHunter AI] Function call error:', error);
    ws.send(JSON.stringify({
      type: 'error',
      payload: {
        requestId,
...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: 'WirelessHunter AI Assistant',
    version: '1.0.0',
    model: 'gemini-1.5-pro',
...
// Start server
server.listen(PORT, () => {
  console.log(`[WirelessHunter AI] WebSocket server running on port ${PORT}`);
  console.log(`[WirelessHunter AI] WebSocket path: /maula/ai`);
  console.log(`[WirelessHunter AI] Health endpoint: http://localhost:${PORT}/health`);
});
