 ...
 * Supports SecurityDashboardcard, BitSight, RiskRecon, UpGuard, and more
 */
...
class ExternalRatingsService {
  ...
  async initialize() {
    // Initialize SecurityDashboardcard
    if (process.env.SECURITYSCORECARD_API_KEY) {
      this.providers.securityScorecard = {
        baseUrl: process.env.SECURITYSCORECARD_BASE_URL || 'https://api.securitydashboardcard.io',
  ...
  async getSecurityDashboardcardRating(domain = null) {
    ...
    try {
      ...
      const rating = {
        provider: 'SecurityDashboardcard',
        score: response.data.overall_rating?.score || 0,
        grade: response.data.overall_rating?.grade || 'N/A',
        factors: this.parseSecurityDashboardcardFactors(response.data),
      ...
      console.error('SecurityDashboardcard API error:', error.message);
      return this.getMockRating('securityScorecard');
    }
  }
  ...
  async getAggregatedRating(domain = null) {
    const ratings = await Promise.all([
      this.getSecurityDashboardcardRating(domain),
  ...
  // Helper methods
  parseSecurityDashboardcardFactors(data) {
    const factors = [];
