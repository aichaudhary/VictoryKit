...
mongoose.connect(MONGODB_URI)
  .then(() => console.log('NetworkMonitor AI Assistant connected to MongoDB'))
...
app.get('/health', (req, res) => {
  res.json({ status: 'NetworkMonitor AI Assistant is running', timestamp: new Date().toISOString() });
});

app.listen(PORT + 1000, () => {
  console.log(`NetworkMonitor AI Assistant health check server running on port ${PORT + 1000}`);
});

// WebSocket server
const wss = new WebSocketServer({ port: PORT });

console.log(`NetworkMonitor AI Assistant WebSocket server running on port ${PORT}`);
...
class NetworkMonitorAI {
  ...
  async analyzeTraffic(query: string, context: any = {}): Promise<string> {
    try {
      ...
      const systemPrompt = `You are NetworkMonitor AI, an advanced network security analysis assistant. You have access to real-time network traffic data, device information, and security alerts.
...
const ai = new NetworkMonitorAI();

wss.on('connection', (ws: WebSocket, req) => {
  console.log('New NetworkMonitor AI client connected');
  ...
  ws.on('close', () => {
    console.log('NetworkMonitor AI client disconnected');
  });

  // Send welcome message
  ws.send(JSON.stringify({
    type: 'welcome',
    message: 'NetworkMonitor AI Assistant connected. Ready to analyze your network traffic.',
    capabilities: [
...
console.log('NetworkMonitor AI Assistant initialized with multi-LLM support');
