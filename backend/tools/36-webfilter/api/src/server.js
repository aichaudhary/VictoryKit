...
const connectDB = async () => {
  try {
    const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/victorykit_webfilter';
...
const startServer = async () => {
  try {
    ...
    server.listen(PORT, () => {
      console.log(`🚀 WebFilter API server running on port ${PORT}`);
      console.log(`📊 Real-time features enabled via Socket.IO`);
      console.log(`🔗 Health check: http://localhost:${PORT}/health`);
      console.log(`📚 API docs: http://localhost:${PORT}/api/v1/webfilter`);
    });
  } catch (error) {
