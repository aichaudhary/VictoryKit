...
// WebFilter AI function declarations
const functions = [
  {
...
console.log(`🛡️ WebFilter AI Assistant running on port ${PORT}`);

wss.on('connection', (ws) => {
  console.log('🔌 Client connected to WebFilter AI');
  ...
  ws.on('message', async (message) => {
    try {
      ...
      if (type === 'function_call') {
        ...
        const result = await executeWebFilterFunction(functionName, parameters);
        ...
        if (functionCall) {
          ...
          const functionResult = await executeWebFilterFunction(
            functionCall.name,
            functionCall.args
          );
  ...
  ws.send(JSON.stringify({
    type: 'connected',
    message: 'Connected to WebFilter AI Assistant',
...
// Graceful shutdown
process.on('SIGINT', () => {
  console.log('\n👋 Shutting down WebFilter AI Assistant...');
  wss.close(() => {
