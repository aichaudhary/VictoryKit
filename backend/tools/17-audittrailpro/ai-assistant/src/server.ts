...
mongoose.connect(MONGODB_URI)
  .then(() => console.log('AuditTrailPro AI Assistant connected to MongoDB'))
...
app.get('/health', (req, res) => {
  res.json({
    status: 'AuditTrailPro AI Assistant is running',
...
app.listen(PORT + 1000, () => {
  console.log(`AuditTrailPro AI Assistant health check server running on port ${PORT + 1000}`);
});

// WebSocket server
const wss = new WebSocketServer({ port: PORT });

console.log(`AuditTrailPro AI Assistant WebSocket server running on port ${PORT}`);
...
class AuditTrailProAI {
  ...
  async analyzeCompliance(query: string, context: any = {}): Promise<string> {
    try {
      ...
      const systemPrompt = `You are AuditTrailPro AI, the FINAL tool in the VictoryKit security suite. You are an advanced compliance and audit analysis assistant with deep expertise in:
...
const ai = new AuditTrailProAI();

wss.on('connection', (ws: WebSocket, req) => {
  console.log('New AuditTrailPro AI client connected');
  ...
  ws.on('close', () => {
    console.log('AuditTrailPro AI client disconnected');
  });

  // Send welcome message
  ws.send(JSON.stringify({
    type: 'welcome',
    message: 'AuditTrailPro AI Assistant connected. Ready to analyze compliance and audit data.',
    capabilities: [
...
console.log('AuditTrailPro AI Assistant initialized with multi-LLM support');
