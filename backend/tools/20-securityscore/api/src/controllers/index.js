...
/**
 * SecurityScore Tool - Unified Controllers
 * 
...
exports.createSecurityScore = async (req, res) => {
  try {
...
exports.getSecurityScores = async (req, res) => {
  try {
    ...
    const scores = await SecurityScore.find(query)
      .populate('improvements.improvementId')
      .skip((page - 1) * limit)
      .limit(Number(limit))
      .sort({ 'overallScore.value': -1 });
    
    const total = await SecurityScore.countDocuments(query);
...
exports.getSecurityScoreById = async (req, res) => {
  try {
...
exports.updateSecurityScore = async (req, res) => {
  try {
    const score = await SecurityScore.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
    if (!score) return res.status(404).json({ success: false, error: 'Score not found' });
    score.calculateOverallScore();
    await score.save();
    res.status(200).json({ success: true, data: score });
...
exports.deleteSecurityScore = async (req, res) => {
  try {
...
exports.getScoreHistory = async (req, res) => {
  try {
    const score = await SecurityScore.findById(req.params.id).select('history');
    if (!score) return res.status(404).json({ success: false, error: 'Score not found' });
...
exports.getTopPerformers = async (req, res) => {
  try {
    ...
    const scores = await SecurityScore.findTopPerformers(limit);
...
exports.compareToBenchmark = async (req, res) => {
  try {
    ...
    const [score, benchmark] = await Promise.all([
      SecurityScore.findById(scoreId),
