...
const logger = winston.createLogger({
  ...
  defaultMeta: { service: 'networkforensics-api' },
  transports: [
...
mongoose.connect(MONGODB_URI)
  .then(() => logger.info('NetworkForensics API connected to MongoDB'))
...
app.get('/health', (req, res) => {
  res.json({
    status: 'NetworkForensics API is running',
...
wss.on('connection', (ws) => {
  logger.info('Real-time client connected to NetworkForensics API');
...
// Start server
app.listen(PORT, () => {
  logger.info(`NetworkForensics API server running on port ${PORT}`);
