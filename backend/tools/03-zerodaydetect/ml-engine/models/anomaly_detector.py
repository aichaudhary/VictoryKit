...
class AnomalyDetector:
    ...
    def detect(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if total_bytes > 0:
            bytes_zscore = (total_bytes - self.stats["bytes_mean"]) / max(self.stats["bytes_std"], 1)
        ...
        if packet_count > 0:
            packet_zscore = (packet_count - self.stats["packet_mean"]) / max(self.stats["packet_std"], 1)
