...
IntelliScout aggregates threat intelligence from 14 trusted sources:
...
IntelliScout maps threats to 12 MITRE ATT&CK tactics:
...
### Backend Setup
```bash
cd backend/tools/02-intelliscout/api
npm install
npm start
```
...
### Frontend Setup
```bash
cd frontend/tools/02-intelliscout
npm install
npm run dev
```
...
### Database Setup
MongoDB automatically creates `intelliscout_db` on first connection.

### Environment Variables
Create `.env` file:
```env
PORT=4002
MONGODB_URI=mongodb://localhost:27017/intelliscout_db
...
### SIEM Integration
Connect IntelliScout to SIEM platforms (Splunk, ELK, QRadar):
...
IntelliScout supports 4 TLP levels:
...
For support, email: support@intelliscout.maula.ai
...
**IntelliScout** - Empowering Security Teams with Actionable Threat Intelligence
