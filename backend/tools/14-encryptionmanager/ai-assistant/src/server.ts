/**
 * EncryptionManager AI Assistant
...
console.log(`🔐 EncryptionManager AI Assistant started on port ${PORT}`);
...
wss.on('connection', (ws: WebSocket) => {
  ...
  const welcomeMessage: AIResponse = {
    id: uuidv4(),
    type: 'welcome',
    content: 'Welcome to EncryptionManager AI Assistant. I can help you with encryption algorithms, key management, security policies, and compliance recommendations.',
    suggestions: [
...
// Graceful shutdown
process.on('SIGINT', () => {
  console.log('\n🛑 Shutting down EncryptionManager AI Assistant...');
  wss.close(() => {
...
<parameter name="filePath">/workspaces/VictoryKit/backend/tools/14-encryptionmanager/ai-assistant/src/server.ts