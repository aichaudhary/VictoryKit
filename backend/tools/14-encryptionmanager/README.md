...
EncryptionManager is a comprehensive enterprise-grade key management and data encryption solution in the VictoryKit security suite. It provides advanced encryption capabilities, secure key storage, automated key rotation, and AI-powered encryption recommendations for protecting sensitive data across the organization.

**Production URL:** `https://encryptionmanager.maula.ai`
...
├─────────────────────────────────────────────────────────────────┤
│  MongoDB Database: encryptionmanager_db                          │
│  ├── Keys Collection                                             │
...
EncryptionManager serves organizations requiring robust data protection:
...
### Backend Setup
```bash
cd backend/tools/14-encryptionmanager/api
npm install
cp .env.example .env
...
### Frontend Setup
```bash
cd frontend/tools/14-encryptionmanager
npm install
npm run dev
```
...
### AI Assistant Setup
```bash
cd backend/tools/14-encryptionmanager/ai-assistant
npm install
npm run dev
```
...
### ML Service Setup
```bash
cd backend/tools/14-encryptionmanager/ml-engine
...
```env
# Database
MONGODB_URI=mongodb://localhost:27017/encryptionmanager_db
...
<parameter name="filePath">/workspaces/VictoryKit/backend/tools/14-encryptionmanager/README.md