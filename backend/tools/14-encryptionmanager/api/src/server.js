...
const MONGODB_URI =
  ...
  "mongodb://localhost:27017/victorykit_encryptionmanager";

mongoose
  .connect(MONGODB_URI)
  .then(() => {
    console.log("✅ Connected to MongoDB - victorykit_encryptionmanager");

    // Initialize WebSocket service
    websocketService.initialize(server);

    server.listen(PORT, () => {
      console.log(`🔐 EncryptionManager API running on port ${PORT}`);
...
🔑 Configured Integrations:
  ...
  .catch((err) => {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      websocketService.initialize(server);
      server.listen(PORT, () => {
        console.log(`🔐 EncryptionManager API (dev mode) on port ${PORT}`);
      });
    }
  });
