...
io.on('connection', (socket) => {
  console.log('EmailGuard client connected:', socket.id);
  ...
  socket.on('disconnect', () => {
    console.log('EmailGuard client disconnected:', socket.id);
  });
});

// MongoDB connection
const MONGODB_URI = process.env.EMAILGUARD_MONGODB_URI || 'mongodb://localhost:27017/emailguard_db';
mongoose.connect(MONGODB_URI)
  .then(() => console.log('MongoDB connected'))
  .catch(err => console.error('MongoDB error:', err));

server.listen(PORT, () => {
  console.log(`EmailGuard API with WebSocket support running on port ${PORT}`);
});
