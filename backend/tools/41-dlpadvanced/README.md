...
**DLPAdvanced** is an enterprise-grade Data Loss Prevention (DLP) solution that provides comprehensive protection against data leaks, insider threats, and compliance violations. The system uses advanced ML models to classify sensitive data, detect policy violations, and prevent unauthorized data transfers.
...
# Backend API (.env)
PORT=4041
MONGODB_URI=mongodb://localhost:27017/dlpadvanced
...
```bash
# Development
cd backend/tools/41-dlpadvanced/api
npm install
npm run dev
...
# Docker
docker build -t dlpadvanced-api .
docker run -p 4041:4041 dlpadvanced-api
```
...
```bash
# Development
cd backend/tools/41-dlpadvanced/ml-engine
...
# Docker
docker build -t dlpadvanced-ml .
docker run -p 8041:8041 dlpadvanced-ml
```
...
```bash
# Development
cd frontend/tools/41-dlpadvanced
npm install
npm run dev
...
# Docker
docker build -t dlpadvanced-frontend .
docker run -p 3041:3041 dlpadvanced-frontend
```
...
services:
  mongodb:
    ...
    environment:
      MONGO_INITDB_DATABASE: dlpadvanced

  ml-engine:
    build: ./backend/tools/41-dlpadvanced/ml-engine
    ports:
      - "8041:8041"
    environment:
      - PYTHONUNBUFFERED=1
  ...
  api:
    build: ./backend/tools/41-dlpadvanced/api
    ports:
      - "4041:4041"
    environment:
      - MONGODB_URI=mongodb://mongodb:27017/dlpadvanced
  ...
  frontend:
    build: ./frontend/tools/41-dlpadvanced
    ports:
      - "3041:3041"
    environment:
...
server {
    ...
    server_name dlpadvanced.maula.ai;
...
- **Postman Collection:** `/docs/postman/dlpadvanced.json`
