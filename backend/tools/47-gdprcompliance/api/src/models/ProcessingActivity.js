...
const processingActivitySchema = new mongoose.Schema({
  ...
  personalDataCategories: [{
    ...
    specialCategoryType: {
      type: String,
      enum: ['health', 'biometric', 'genetic', 'racial_ethnic', 'political', 'religious', 'trade_union', 'sex_life']
    }
  }],
  ...
  recipientCategories: [{
    category: {
      type: String,
      enum: ['internal_departments', 'group_companies', 'processors', 'third_party_services', 'public_authorities', 'professional_advisors', 'other']
    },
...
processingActivitySchema.methods.requiresDPIA = function() {
  ...
  const hasSystematicMonitoring = this.purposes.some(p => 
    p.purpose.includes('monitoring') || p.purpose.includes('tracking')
  );
