/**
 * BiometricAI AI Assistant Server
...
// AI Function definitions for BiometricAI
...
// System prompt for BiometricAI
const systemPrompt = `You are the AI assistant for BiometricAI, a comprehensive multi-modal biometric authentication system.
...
console.log(`[BiometricAI AI] WebSocket server starting on port ${PORT}...`);

wss.on('connection', (ws) => {
  const connectionId = uuidv4();
  connections.set(connectionId, ws);
  
  console.log(`[BiometricAI AI] Client connected: ${connectionId}`);
  
  // Initialize chat session
  const chat = model.startChat({
    history: [
      { role: 'user', parts: [{ text: 'Initialize as BiometricAI assistant.' }] },
      { role: 'model', parts: [{ text: 'BiometricAI assistant initialized. I can help you with biometric quality analysis, spoof detection, liveness verification, match scoring, behavioral analysis, risk assessment, template optimization, authentication advice, compliance checking, and anomaly detection. How can I assist you?' }] }
    ],
    generationConfig: {
  ...
  ws.send(JSON.stringify({
    type: 'connected',
    connectionId,
    message: 'Connected to BiometricAI AI Assistant',
  ...
  ws.on('message', async (data) => {
    try {
      ...
      console.error(`[BiometricAI AI] Error processing message:`, error);
      ws.send(JSON.stringify({
        type: 'error',
        error: error.message
      }));
  ...
  ws.on('close', () => {
    console.log(`[BiometricAI AI] Client disconnected: ${connectionId}`);
    connections.delete(connectionId);
    chatSessions.delete(connectionId);
  });
  
  ws.on('error', (error) => {
    console.error(`[BiometricAI AI] WebSocket error:`, error);
  });
});
...
async function handleChatMessage(connectionId, ws, messageId, content, stream = true) {
  ...
  try {
    ...
    console.error(`[BiometricAI AI] Chat error:`, error);
    ws.send(JSON.stringify({
      type: 'error',
      id: messageId,
      error: error.message
...
console.log(`[BiometricAI AI] Server running on ws://localhost:${PORT}`);
console.log(`[BiometricAI AI] Available functions: ${biometricFunctions.map(f => f.name).join(', ')}`);
