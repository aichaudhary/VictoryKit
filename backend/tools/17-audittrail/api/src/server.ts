...
const logger = winston.createLogger({
  ...
  defaultMeta: { service: 'audittrail-api' },
  transports: [
...
mongoose.connect(MONGODB_URI)
  .then(() => logger.info('AuditTrail API connected to MongoDB'))
...
app.get('/health', (req, res) => {
  res.json({
    status: 'AuditTrail API is running',
...
wss.on('connection', (ws) => {
  logger.info('Real-time client connected to AuditTrail API');
...
// Start server
app.listen(PORT, () => {
  logger.info(`AuditTrail API server running on port ${PORT}`);
  logger.info(`WebSocket server running on port ${WS_PORT}`);
  logger.info('FINAL TOOL - AuditTrail is now operational');
});
