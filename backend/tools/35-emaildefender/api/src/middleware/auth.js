...
const authenticate = (req, res, next) => {
  try {
    ...
    jwt.verify(token, process.env.EMAILGUARD_JWT_SECRET || 'emaildefender-secret-key', (err, user) => {
      if (err) {
...
// Security analyst middleware
const requireAnalyst = authorize('admin', 'analyst', 'superadmin');
...
const corsOptions = {
  origin: function (origin, callback) {
    const allowedOrigins = [
      'http://localhost:3035', // EmailDefender frontend
      'http://localhost:3000', // Main dashboard
      process.env.EMAILGUARD_FRONTEND_URL
    ].filter(Boolean);
