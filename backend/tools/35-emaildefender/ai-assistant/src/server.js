/**
 * EmailDefender AI Assistant Server
...
import { executeEmailDefenderFunction } from './functionExecutor.js';
...
// AI Function definitions for EmailDefender
...
// System prompt for EmailDefender
const systemPrompt = `You are the AI assistant for EmailDefender, an enterprise email security and protection platform.
...
console.log(`[EmailDefender AI] WebSocket server starting on port ${PORT}...`);

wss.on('connection', (ws) => {
  const connectionId = uuidv4();
  connections.set(connectionId, ws);
  
  console.log(`[EmailDefender AI] Client connected: ${connectionId}`);
  
  // Initialize chat session
  const chat = model.startChat({
    history: [
      { role: 'user', parts: [{ text: 'Initialize as EmailDefender assistant.' }] },
      { role: 'model', parts: [{ text: 'EmailDefender assistant initialized. I can help you with email threat analysis, phishing detection, malware scanning, BEC prevention, sender reputation assessment, policy recommendations, and threat landscape summaries. How can I assist you with email security today?' }] }
    ],
    generationConfig: {
  ...
  ws.send(JSON.stringify({
    type: 'connected',
    connectionId,
    message: 'Connected to EmailDefender AI Assistant',
  ...
  ws.on('message', async (data) => {
    try {
      ...
      console.error(`[EmailDefender AI] Error processing message:`, error);
      ws.send(JSON.stringify({
        type: 'error',
        error: error.message
      }));
  ...
  ws.on('close', () => {
    console.log(`[EmailDefender AI] Client disconnected: ${connectionId}`);
    connections.delete(connectionId);
    chatSessions.delete(connectionId);
  });
  
  ws.on('error', (error) => {
    console.error(`[EmailDefender AI] WebSocket error:`, error);
  });
});
...
async function handleChatMessage(connectionId, ws, messageId, content, stream = true) {
  ...
  try {
    ...
    if (functionMatch) {
      ...
      const result = await executeEmailDefenderFunction(functionMatch.name, functionMatch.parameters);
    ...
    console.error(`[EmailDefender AI] Chat error:`, error);
    ws.send(JSON.stringify({
      type: 'error',
      id: messageId,
      error: error.message
...
async function handleFunctionCall(ws, id, functionCall) {
  ...
  try {
    const result = await executeEmailDefenderFunction(name, parameters);
...
console.log(`[EmailDefender AI] Server running on ws://localhost:${PORT}`);
console.log(`[EmailDefender AI] Available functions: ${emailGuardFunctions.map(f => f.name).join(', ')}`);
