/**
 * BotMitigation AI Assistant - WebSocket Server
...
import { executeBotMitigationFunction } from './functions/botmitigationFunctions';
...
const SYSTEM_PROMPT = `You are Maula, the AI assistant for BotMitigation - an AI-powered bot detection and mitigation platform.
...
wss.on('connection', (ws: WebSocket) => {
  ...
  ws.send(JSON.stringify({ type: 'connected', clientId, message: 'Connected to BotMitigation AI' }));
...
async function handleChat(client: Client, content: string) {
  ...
  let responseText = genAI 
    ...
    : `I'm Maula, BotMitigation AI. Configure your API key for full functionality. Your question: "${content}"`;
...
async function handleFunctionCall(client: Client, functionName: string, parameters: Record<string, unknown>, requestId?: string) {
  try {
    const result = await executeBotMitigationFunction(functionName, parameters);
...
console.log(`[BotMitigation AI] WebSocket server listening on port ${PORT}`);
export { wss };
