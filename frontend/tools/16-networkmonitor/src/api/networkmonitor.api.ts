/**
 * NetworkMonitor API Client - Tool 16 - Real-Time Network Traffic Analysis
 * Enhanced API client with full CRUD operations and WebSocket support
 */

const API_BASE_URL = import.meta.env.VITE_NETWORKMONITOR_API_URL || 'http://localhost:4016/api';
const WS_URL = import.meta.env.VITE_NETWORKMONITOR_WS_URL || 'ws://localhost:4016/ws';
...
class WebSocketManager {
  ...
  connect(): void {
    ...
    try {
      ...
      this.ws.onopen = () => { console.log('[WS] Connected to NetworkMonitor'); this.connected = true; this.reconnectAttempts = 0; };
