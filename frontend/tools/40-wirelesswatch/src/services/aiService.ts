...
class WirelessWatchAIService {
  ...
  async connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log(`[WirelessWatch AI] Connecting to ${wsUrl}`);
        
        this.ws = new WebSocket(wsUrl);
        
        this.ws.onopen = () => {
          console.log('[WirelessWatch AI] Connected');
        ...
        this.ws.onerror = (event) => {
          console.error('[WirelessWatch AI] WebSocket error:', event);
        ...
        this.ws.onclose = (event) => {
          console.log(`[WirelessWatch AI] Disconnected: ${event.code} ${event.reason}`);
    ...
    try {
      ...
      switch (parsed.type) {
        ...
        case 'stream':
          ...
          break;
        default:
          console.warn('[WirelessWatch AI] Unknown message type:', parsed.type);
      }
    } catch (error) {
      console.error('[WirelessWatch AI] Failed to parse message:', error);
    }
  }
  ...
  private attemptReconnect(): void {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      console.error('[WirelessWatch AI] Max reconnection attempts reached');
      return;
    }
    ...
    console.log(`[WirelessWatch AI] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`);
...
export const wirelessWatchAI = new WirelessWatchAIService();

// Export class for testing
export { WirelessWatchAIService };
