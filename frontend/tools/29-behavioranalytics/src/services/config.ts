...
let configCache: BehaviorAnalyticsConfig | null = null;

export async function loadConfig(): Promise<BehaviorAnalyticsConfig> {
  if (configCache) {
    return configCache;
  }

  try {
    const response = await fetch('/behavioranalytics-config.json');
    if (!response.ok) {
      throw new Error(`Failed to load config: ${response.statusText}`);
    }
    configCache = await response.json();
    return configCache as BehaviorAnalyticsConfig;
  } catch (error) {
    console.error('Error loading BehaviorAnalytics config:', error);
    throw error;
  }
}

export function getConfig(): BehaviorAnalyticsConfig | null {
  return configCache;
}
