...
const Dashboard: React.FC = () => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'completed': return <CheckCircle size={16} className="text-securecode-400" />;
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      ...
      case 'down': return <TrendingDown size={14} className="text-securecode-400" />;
      default: return <span className="text-gray-500">—</span>;
    }
  };

  return (
    <div className="min-h-screen bg-securecode-darker p-6">
      {/* Header */}
      <div className="mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-securecode-dark/50 rounded-xl p-6 border border-securecode-800/50 hover:border-securecode-600/50 transition-all hover:shadow-glow-green">
          <div className="flex items-center justify-between mb-4">
            <div className="p-3 bg-securecode-600/20 rounded-lg">
              <FileSearch size={24} className="text-securecode-400" />
            </div>
            <span className="text-xs text-securecode-400 font-semibold bg-securecode-600/20 px-2 py-1 rounded">+12%</span>
          </div>
        ...
        <div className="bg-securecode-dark/50 rounded-xl p-6 border border-securecode-800/50 hover:border-yellow-500/50 transition-all">
          <div className="flex items-center justify-between mb-4">
        ...
        <div className="bg-securecode-dark/50 rounded-xl p-6 border border-securecode-800/50 hover:border-blue-500/50 transition-all">
          <div className="flex items-center justify-between mb-4">
            <div className="p-3 bg-blue-500/20 rounded-lg">
              <Code size={24} className="text-blue-400" />
            </div>
            <span className="text-xs text-securecode-400 font-semibold bg-securecode-600/20 px-2 py-1 rounded">A+</span>
          </div>
          <h3 className="text-gray-400 text-sm mb-1">Code Quality</h3>
          <p className="text-3xl font-bold text-white">{stats.codeQuality}%</p>
        </div>

        <div className="bg-securecode-dark/50 rounded-xl p-6 border border-securecode-800/50 hover:border-securecode-500/50 transition-all">
          <div className="flex items-center justify-between mb-4">
            <div className="p-3 bg-securecode-500/20 rounded-lg">
              <Shield size={24} className="text-securecode-400" />
            </div>
            <span className="text-xs text-securecode-400 font-semibold bg-securecode-600/20 px-2 py-1 rounded">95%</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Recent Scans */}
        <div className="lg:col-span-2 bg-securecode-dark/50 rounded-xl border border-securecode-800/50">
          <div className="p-6 border-b border-securecode-800/50">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold text-white">Recent Scans</h2>
              <button className="text-sm text-securecode-400 hover:text-securecode-300 transition-colors">View All</button>
            </div>
          </div>
          <div className="divide-y divide-securecode-800/50">
            {recentScans.map((scan) => (
              <div key={scan.id} className="p-4 hover:bg-securecode-800/20 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-500 bg-securecode-800/50 px-2 py-0.5 rounded">{scan.branch}</span>
                      </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <button className="p-2 hover:bg-securecode-700/30 rounded-lg text-gray-400 hover:text-securecode-400 transition-colors">
                      <Eye size={16} />
                    </button>
                  </div>
                </div>
        ...
        <div className="bg-securecode-dark/50 rounded-xl border border-securecode-800/50">
          <div className="p-6 border-b border-securecode-800/50">
            <h2 className="text-xl font-semibold text-white">Vulnerability Types</h2>
          </div>
          <div className="p-4 space-y-3">
            {vulnerabilityCategories.map((cat, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-securecode-800/20 hover:bg-securecode-800/30 transition-colors">
                <div className="flex items-center gap-3">
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <button className="p-4 bg-securecode-gradient rounded-xl text-white font-semibold flex items-center justify-center gap-2 hover:shadow-glow-green transition-all">
          <Play size={20} />
          Start New Scan
        </button>
        <button className="p-4 bg-securecode-dark/50 border border-securecode-600/50 rounded-xl text-securecode-400 font-semibold flex items-center justify-center gap-2 hover:bg-securecode-800/30 transition-all">
          <GitBranch size={20} />
          Connect Repository
        </button>
        <button className="p-4 bg-securecode-dark/50 border border-securecode-600/50 rounded-xl text-securecode-400 font-semibold flex items-center justify-center gap-2 hover:bg-securecode-800/30 transition-all">
