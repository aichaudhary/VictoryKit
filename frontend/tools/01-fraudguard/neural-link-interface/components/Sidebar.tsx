...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside
      className={`absolute top-0 left-0 h-full w-[85%] sm:w-64 md:w-72 bg-[#0a0a0a]/98 backdrop-blur-xl border-r border-gray-800/50 p-4 sm:p-6 transition-transform duration-500 ease-out z-[60] flex flex-col shadow-[20px_0_60px_rgba(0,0,0,0.5)] ${
        ...
        {sessions.map((s) => (
          <div key={s.id} className="relative group">
            <button
              ...
              className={`w-full text-left flex items-center justify-between p-4 rounded-lg border transition-all duration-300 ${
                s.active
                  ? "bg-green-500/10 border-green-500/40 text-white shadow-[inset_0_0_20px_rgba(34,197,94,0.1)]"
                  : "border-gray-900 text-gray-600 hover:border-gray-800 hover:bg-gray-800/20 hover:text-gray-400"
              }`}
            >
              <div className="flex flex-col gap-1 overflow-hidden">
                <span className="text-[11px] font-bold font-mono truncate uppercase tracking-wider">
                  {s.name}
                </span>
                <div className="flex items-center gap-2">
                  <span
                    className={`w-1 h-1 rounded-full ${
                      s.active ? "bg-green-500 animate-pulse" : "bg-gray-800"
                    }`}
                  ></span>
                  <span className="text-[8px] opacity-60 uppercase font-mono">
                    {s.messages.length} pkts
                  </span>
                </div>
              </div>
              <div
            ...
            {activeMenu === s.id && (
              <div className="absolute right-0 top-full mt-1 w-48 bg-[#0d0d0d] border border-gray-800 rounded-md shadow-2xl z-[70] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="p-1">
                  <button
                    onClick={(e) => handleAction(e, s.id, "share")}
                    className="w-full flex items-center gap-3 px-3 py-2 text-[10px] text-gray-400 hover:bg-emerald-500/10 hover:text-emerald-400 rounded transition-colors uppercase font-mono"
                  >
                    <Share2 size={12} /> Share Uplink
                  </button>
                  <button
                    onClick={(e) => handleAction(e, s.id, "copy")}
                    className="w-full flex items-center gap-3 px-3 py-2 text-[10px] text-gray-400 hover:bg-cyan-500/10 hover:text-cyan-400 rounded transition-colors uppercase font-mono"
                  >
                    <Copy size={12} /> Copy Manifest
                  </button>
                  <button
                    onClick={(e) => handleAction(e, s.id, "download")}
                    className="w-full flex items-center gap-3 px-3 py-2 text-[10px] text-gray-400 hover:bg-purple-500/10 hover:text-purple-400 rounded transition-colors uppercase font-mono"
                  >
                    <Download size={12} /> Sync Bundle (ZIP)
                  </button>
                  <button
                    onClick={(e) => handleAction(e, s.id, "delete")}
                    className="w-full flex items-center gap-3 px-3 py-2 text-[10px] text-red-900 hover:bg-red-500/10 hover:text-red-500 rounded transition-colors uppercase font-mono mt-1 border-t border-gray-800/50 pt-2"
                  >
