  ...
  const handleSend = async (text: string) => {
    ...
    if (result.toolResults && result.toolResults.length > 0) {
      result.toolResults.forEach((toolResult: any) => {
        if (toolResult.result.display) {
          // Update canvas with tool display results
          const display = toolResult.result.display;
          if (display.type === "card" || display.type === "chart") {
            updateActiveSettings({
              ...activeSession.settings,
              workspaceMode: "CANVAS",
              canvas: {
                ...activeSession.settings.canvas,
                content: JSON.stringify(display.content, null, 2),
                type: display.type === "card" ? "text" : "code",
                title: `TOOL_RESULT_${toolResult.tool.toUpperCase()}`,
              },
            });
          }
        }
