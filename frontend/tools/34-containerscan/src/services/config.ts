// ContainerScan Configuration Service
// Loads and manages configuration from containerscan-config.json
...
export async function loadConfig(): Promise<BiometricConfig> {
  ...
  configPromise = (async () => {
    try {
      const response = await fetch('/containerscan-config.json');
      if (!response.ok) {
      ...
      console.error('[ContainerScan] Failed to load configuration:', error);
      throw error;
    }
  })();
