...
export interface RiskQuantifyment {
...
class ContainerScanService {
  ...
  private connect(): void {
    try {
      ...
      this.ws.onopen = () => {
        console.log('[ContainerScan] WebSocket connected');
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          console.error('[ContainerScan] Failed to parse message:', error);
        }
      };

      this.ws.onerror = (error) => {
        console.error('[ContainerScan] WebSocket error:', error);
        this.errorCallbacks.forEach(cb => cb(new Error('WebSocket error')));
      };

      this.ws.onclose = () => {
        console.log('[ContainerScan] WebSocket disconnected');
        this.isConnected = false;
        this.connectionCallbacks.forEach(cb => cb(false));
        this.attemptReconnect();
      };
    } catch (error) {
      console.error('[ContainerScan] Failed to connect:', error);
      this.attemptReconnect();
    }
  }
  ...
  private attemptReconnect(): void {
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.log(`[ContainerScan] Reconnecting (${this.reconnectAttempts}/${this.maxReconnectAttempts})...`);
...
// Singleton instance
const aiService = new ContainerScanService();

export default aiService;

export {
  ContainerScanService,
  aiService,
};
