...
```bash
# 1. Frontend Setup
cd frontend/tools/35-emailguard
npm install
npm run dev  # Runs on port 3035

# 2. Backend API Setup
cd backend/tools/35-emailguard/api
npm install
npm start    # Runs on port 4035

# 3. AI Assistant Setup
cd backend/tools/35-emailguard/ai-assistant
npm install
npm start    # Runs on port 6035

# 4. ML Engine Setup
cd backend/tools/35-emailguard/ml-engine
...
```bash
PORT=4035
MONGODB_URI=mongodb://localhost:27017/emailguard_db
...
EmailGuard includes 10 specialized AI functions powered by Gemini 1.5 Pro:
...
Configuration is managed through `emailguard-config.json`:
...
EmailGuard supports compliance with:
...
```javascript
const response = await emailguardAPI.processEmail({
...
### Check Quarantine
```javascript
const quarantine = await emailguardAPI.getQuarantine({
  page: 1,
  limit: 20,
  status: 'quarantined'
});
...
```javascript
const policy = await emailguardAPI.createPolicy({
...
```javascript
const stats = await emailguardAPI.getRealtimeStats();
...
**EmailGuard - Protecting Your Email Infrastructure with AI-Powered Intelligence** 🛡️✉️
