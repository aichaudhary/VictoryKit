...
```bash
# 1. Frontend Setup
cd frontend/tools/37-dnsshield
npm install
...
cd backend/tools/37-dnsshield/ai-assistant
npm install
npm start    # Runs on port 6037

# 4. DNS Resolver Setup
cd backend/tools/37-dnsshield/resolver
npm install
...
Configuration is managed through `dnsshield-config.json`:
...
### Query DNS
```javascript
const result = await dnsshieldAPI.queryDomain({
  domain: 'example.com',
  queryType: 'A',
  clientIP: '192.168.1.100'
});
...
### Block Domain
```javascript
await dnsshieldAPI.blockDomain({
  domain: 'malicious.com',
  category: 'malware',
  reason: 'Listed in URLhaus',
...
### Check Reputation
```javascript
const reputation = await dnsshieldAPI.checkDomainReputation('example.com');
console.log(`Reputation Score: ${reputation.score}`);
```

### Generate Report
```javascript
const report = await dnsshieldAPI.generateThreatReport({
  reportType: 'weekly',
  startDate: '2026-01-01',
  endDate: '2026-01-07'
});
...
systemctl status dnsshield-resolver

# Test DNS resolution
dig @localhost example.com

# Check logs
tail -f /var/log/dnsshield/resolver.log
```
...
dnsshield-resolver --update-trust-anchors
