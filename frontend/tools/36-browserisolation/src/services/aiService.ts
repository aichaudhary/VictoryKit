...
class BrowserIsolationAIService {
  ...
  private connect(): void {
    ...
    try {
      ...
      this.ws.onopen = () => {
        console.log('[BrowserIsolation AI] Connected to AI Assistant');
        this.isConnecting = false;
        this.reconnectAttempts = 0;
      };

      this.ws.onclose = () => {
        console.log('[BrowserIsolation AI] Disconnected from AI Assistant');
        this.isConnecting = false;
        this.attemptReconnect();
      };

      this.ws.onerror = (error) => {
        console.error('[BrowserIsolation AI] WebSocket error:', error);
        this.isConnecting = false;
      };
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          console.error('[BrowserIsolation AI] Failed to parse message:', error);
        }
      };
    } catch (error) {
      console.error('[BrowserIsolation AI] Connection error:', error);
      this.isConnecting = false;
      this.attemptReconnect();
    }
  }
  ...
  private attemptReconnect(): void {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      console.error('[BrowserIsolation AI] Max reconnection attempts reached');
      return;
    }
    ...
    console.log(`[BrowserIsolation AI] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})`);
...
export const aiService = new BrowserIsolationAIService();
