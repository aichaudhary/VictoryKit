...
const BlueTeamAITool: React.FC = () => {
  const [isActive, setIsActive] = useState(true);
  const [stats, setStats] = useState<Stats>({ total: 14863, active: 28747 % 500, blocked: 15918 % 1000, pending: 12617 % 50 });
      ...
      <header className="border-b border-blue-500/20 bg-black/30 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <a href="https://maula.ai/#tool-26" className="p-2 hover:bg-white/10 rounded-lg transition-colors" title="Back to Maula.ai">
            ...
            <div>
              <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-400 to-blue-200 bg-clip-text text-transparent">Blue Team AI</h1>
              <p className="text-sm text-gray-400">Defensive Security Operations</p>
            </div>
          </div>
          ...
          <div className="flex gap-1">
            {['dashboard', 'monitor', 'rules', 'analytics', 'settings'].map((tab) => (
              <button key={tab} onClick={() => setSelectedTab(tab)}
                className={`px-6 py-3 text-sm font-medium capitalize ${selectedTab === tab ? 'text-blue-400 border-b-2 border-blue-500 bg-blue-500/10' : 'text-gray-400 hover:text-white'}`}>
                {tab}
              </button>
            ))}
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-2 bg-gray-800/50 rounded-xl border border-blue-500/20">
            <div className="p-4 border-b border-gray-700/50">
              <h2 className="text-lg font-semibold flex items-center gap-2">
                <Bell className="w-5 h-5 text-blue-400" />Recent Alerts
              </h2>
            </div>
          ...
          <div className="bg-gray-800/50 rounded-xl border border-blue-500/20">
            <div className="p-4 border-b border-gray-700/50">
              <h2 className="text-lg font-semibold flex items-center gap-2"><Zap className="w-5 h-5 text-blue-400" />Quick Actions</h2>
            </div>
            <div className="p-4 space-y-3">
              <button className="w-full p-3 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 rounded-lg flex items-center gap-3">
                <Search className="w-5 h-5 text-blue-400" /><span>Run Scan</span>
              </button>
