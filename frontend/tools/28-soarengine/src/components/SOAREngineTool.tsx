...
const SOAREngineTool: React.FC = () => {
  const [isActive, setIsActive] = useState(true);
  const [stats, setStats] = useState<Stats>({ total: 16212, active: 28998 % 500, blocked: 6715 % 1000, pending: 2188 % 50 });
      ...
      <header className="border-b border-violet-500/20 bg-black/30 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <a href="https://maula.ai/#tool-28" className="p-2 hover:bg-white/10 rounded-lg transition-colors" title="Back to Maula.ai">
              <ArrowLeft className="w-6 h-6 text-violet-400" />
            </a>
            <div className="p-2 bg-violet-500/20 rounded-lg">
              <Cpu className="w-8 h-8 text-violet-400" />
            </div>
            <div>
              <h1 className="text-2xl font-bold bg-gradient-to-r from-violet-400 to-violet-200 bg-clip-text text-transparent">SOAR Engine</h1>
              <p className="text-sm text-gray-400">Security Orchestration & Response</p>
            </div>
          </div>
