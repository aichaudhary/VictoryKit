// KubeArmor Configuration Loader
// Loads and manages tool configuration

import configData from '../../kubearmor-config.json';

export interface KubeArmorConfig {
  toolName: string;
  tagline: string;
  subdomain: string;
  port: number;
...
class ConfigService {
  private config: KubeArmorConfig;
  
  constructor() {
    this.config = configData as KubeArmorConfig;
  }
  
  getConfig(): KubeArmorConfig {
    return this.config;
  }
