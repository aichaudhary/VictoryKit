...
declare const import_meta_env: { VITE_CODESENTINEL_API_URL?: string } | undefined;

const getApiUrl = (): string => {
  try {
    // @ts-ignore - Vite injects import.meta.env at build time
    if (typeof import.meta !== 'undefined' && import.meta.env?.VITE_CODESENTINEL_API_URL) {
      // @ts-ignore
      return import.meta.env.VITE_CODESENTINEL_API_URL;
    }
  } catch {
...
class CodeSentinelAPI {
  ...
  private async request<T>(
    ...
    try {
      ...
      console.error('CodeSentinel API Error:', error);
      return {
        success: false,
...
export const codesentinelAPI = new CodeSentinelAPI();

// Export types for use in components
export default codesentinelAPI;
