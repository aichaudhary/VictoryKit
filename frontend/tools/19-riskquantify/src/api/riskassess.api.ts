/**
 * RiskQuantify API Client - Tool 19 - Enhanced Risk Assessment & Management
 * Features: AI Analysis, Real-time Collaboration, Threat Intelligence, Compliance Integration
 */
const API_BASE_URL = import.meta.env.VITE_RISKQUANTIFY_API_URL || 'http://localhost:4019/api/v1';
...
class RiskQuantifyApi {
  ...
  async getDashboard(): Promise<ApiResponse<RiskDashboard>> {
    return this.request('/riskquantify/dashboard');
  }

  async getRisks(): Promise<ApiResponse<Risk[]>> {
    return this.request('/riskquantify/risks');
  }

  async getRiskMatrix(): Promise<ApiResponse<RiskMatrix>> {
    return this.request('/riskquantify/matrix');
  }
...
export const riskAssessApi = new RiskQuantifyApi();
