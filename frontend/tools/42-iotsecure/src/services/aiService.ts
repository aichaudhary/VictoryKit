...
class IoTSecureAIService {
  ...
  connect(): Promise<AISession> {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onopen = () => {
          console.log('[IoTSecure AI] Connected');
        ...
        this.ws.onmessage = (event) => {
          try {
            ...
            console.error('[IoTSecure AI] Parse error:', error);
          }
        };

        this.ws.onerror = (error) => {
          console.error('[IoTSecure AI] WebSocket error:', error);
        ...
        this.ws.onclose = () => {
          console.log('[IoTSecure AI] Disconnected');
          if (this.session) {
  ...
  private handleMessage(message: { type: string; payload: Record<string, unknown> }, resolve?: (session: AISession) => void) {
    switch (message.type) {
      ...
      case 'function_result':
        console.log('[IoTSecure AI] Function result:', message.payload);
        break;
  ...
  private attemptReconnect() {
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.log(`[IoTSecure AI] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`);
  ...
  sendMessage(content: string, context?: Record<string, unknown>): string {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      throw new Error('Not connected to IoTSecure AI');
    }
  ...
  // Convenience methods for IoTSecure AI functions
...
export const iotSecureAI = new IoTSecureAIService();
