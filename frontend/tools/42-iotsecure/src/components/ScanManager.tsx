...
export const ScanManager: React.FC<ScanManagerProps> = ({ onScanComplete }) => {
  ...
  const fetchScans = async () => {
    try {
      setLoading(true);
      const response = await iotSecureAPI.scans.getAll({ limit: 20 });
  ...
  const fetchRunningScans = async () => {
    try {
      const response = await iotSecureAPI.scans.getRunning();
  ...
  const startScan = async () => {
    try {
      ...
      switch (scanType) {
        case 'discovery':
          response = await iotSecureAPI.scans.startDiscovery(networks);
          break;
        case 'vulnerability':
          response = await iotSecureAPI.scans.startVulnerability([]);
          break;
        default:
          response = await iotSecureAPI.scans.startQuick(networks);
      }
  ...
  const cancelScan = async (scanId: string) => {
    try {
      await iotSecureAPI.scans.cancel(scanId);
      fetchRunningScans();
      fetchScans();
    } catch (err) {
