...
class VPNGuardianAPI {
  ...
  constructor() {
    ...
    this.api.interceptors.request.use(
      (config) => {
        const token = localStorage.getItem('vpnguardian_token');
        if (token) {
    ...
    this.api.interceptors.response.use(
      ...
      (error) => {
        if (error.response?.status === 401) {
          localStorage.removeItem('vpnguardian_token');
  ...
  connectWebSocket(onMessage: (data: any) => void): WebSocket | null {
    try {
      ...
      ws.onopen = () => {
        console.log('VPNGuardian WebSocket connected');
      };
      ...
      ws.onclose = () => {
        console.log('VPNGuardian WebSocket disconnected');
...
export const vpnGuardianAPI = new VPNGuardianAPI();