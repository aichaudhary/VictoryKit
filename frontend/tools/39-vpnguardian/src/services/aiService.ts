...
class VPNGuardianAIService {
  ...
  async connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log(`[VPNGuardian AI] Connecting to ${wsUrl}`);
        
        this.ws = new WebSocket(wsUrl);
        
        this.ws.onopen = () => {
          console.log('[VPNGuardian AI] Connected');
        ...
        this.ws.onerror = (event) => {
          console.error('[VPNGuardian AI] WebSocket error:', event);
        ...
        this.ws.onclose = (event) => {
          console.log(`[VPNGuardian AI] Disconnected: ${event.code} ${event.reason}`);
    ...
    try {
      ...
      switch (parsed.type) {
        ...
        case 'stream':
          ...
          break;
        default:
          console.warn('[VPNGuardian AI] Unknown message type:', parsed.type);
      }
    } catch (error) {
      console.error('[VPNGuardian AI] Failed to parse message:', error);
    }
  }
  ...
  private attemptReconnect(): void {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      console.error('[VPNGuardian AI] Max reconnection attempts reached');
      return;
    }
    ...
    console.log(`[VPNGuardian AI] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`);
...
export const vpnGuardianAI = new VPNGuardianAIService();

// Export class for testing
export { VPNGuardianAIService };
