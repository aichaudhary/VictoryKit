// EmailDefender Configuration Service
// Loads and manages configuration from emaildefender-config.json

// Configuration Types
export interface EmailDefenderConfig {
  tool: ToolInfo;
...
// Configuration state
let config: EmailDefenderConfig | null = null;
let configPromise: Promise<EmailDefenderConfig> | null = null;

// Load configuration
export async function loadConfig(): Promise<EmailDefenderConfig> {
  if (config) {
    return config;
  }
  ...
  configPromise = (async () => {
    try {
      const response = await fetch('/emaildefender-config.json');
      if (!response.ok) {
      ...
      console.error('[EmailDefender] Failed to load configuration:', error);
      throw error;
    }
  })();
...
export function getConfig(): EmailDefenderConfig {
  if (!config) {
