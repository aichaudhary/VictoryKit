...
class EmailDefenderAIService {
  ...
  private connect(): void {
    try {
      ...
      this.ws.onopen = () => {
        console.log('[EmailDefender AI] WebSocket connected');
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          console.error('[EmailDefender AI] Failed to parse message:', error);
        }
      };
      ...
      this.ws.onclose = () => {
        console.log('[EmailDefender AI] WebSocket disconnected');
        this.isConnected = false;
        this.connectionCallbacks.forEach(cb => cb(false));
        this.attemptReconnect();
      };
    } catch (error) {
      console.error('[EmailDefender AI] Failed to connect:', error);
      this.attemptReconnect();
    }
  }
  ...
  private attemptReconnect(): void {
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.log(`[EmailDefender AI] Reconnecting (${this.reconnectAttempts}/${this.maxReconnectAttempts})...`);
...
// Singleton instance
const aiService = new EmailDefenderAIService();

export default aiService;

export {
  EmailDefenderAIService,
  aiService,
};
