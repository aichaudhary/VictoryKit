/**
 * SecurityDashboard API Client v2.0 - Enhanced Security Posture Scoring
 */
const API_BASE_URL = import.meta.env.VITE_SECURITYDASHBOARD_API_URL || 'http://localhost:4020/api';
const WS_URL = import.meta.env.VITE_SECURITYDASHBOARD_WS_URL || 'ws://localhost:4120';
...
export interface ScoreDashboard {
  currentScore: SecurityDashboard;
...
class SecurityDashboardApi {
  ...
  async getScore(id?: string): Promise<ApiResponse<SecurityDashboard>> {
  ...
  async calculateScore(id: string): Promise<ApiResponse<SecurityDashboard>> {
  ...
  async getAIRecommendations(securityScore: SecurityDashboard, context?: any): Promise<ApiResponse<Recommendation[]>> {
...
export const securityScoreApi = new SecurityDashboardApi();
