...
import { useAPIShieldStore } from '../stores/apiGuardStore';
...
class WebSocketService {
  ...
  private setupEventHandlers() {
    ...
    this.socket.on('connect', () => {
      ...
      useAPIShieldStore.getState().setConnectionStatus('connected');
    });

    this.socket.on('disconnect', (reason) => {
      console.log('WebSocket disconnected:', reason);
      useAPIShieldStore.getState().setConnectionStatus('disconnected');
    });

    this.socket.on('connect_error', (error) => {
      console.error('WebSocket connection error:', error);
      this.reconnectAttempts++;
      useAPIShieldStore.getState().setConnectionStatus('error');
    });

    // Real-time API request events
    this.socket.on('api_request', (data: RealTimeEvent) => {
      useAPIShieldStore.getState().addRealTimeEvent({
        ...data,
    ...
    this.socket.on('anomaly_detected', (data: Anomaly) => {
      const store = useAPIShieldStore.getState();
    ...
    this.socket.on('metrics_update', (data: { apiId: string; requests: number; errors: number; latency: number }) => {
      useAPIShieldStore.getState().updateAPIMetrics(data.apiId, {
    ...
    this.socket.on('score_update', (data: { apiId: string; score: number; grade: string }) => {
      useAPIShieldStore.getState().updateAPIScore(data.apiId, data.score, data.grade);
    });
  }
