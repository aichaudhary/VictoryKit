...
const Dashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'marketplace' | 'fleet' | 'analytics' | 'billing' | 'logs' | 'profile'
  >('overview');
  ...
  const SidebarItem = ({
    ...
    >
      ...
      <span
        className={`${activeTab === id ? 'text-purple-400' : 'text-white/40 group-hover:text-purple-400'}`}
      >
        {icon}
      </span>
    ...
    <div className="flex h-screen bg-[#02000a] text-white overflow-hidden font-sans selection:bg-purple-500/30">
      ...
      <aside className="w-80 border-r border-white/5 bg-[#040412]/80 backdrop-blur-3xl flex flex-col p-8 z-[100] relative">
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="text-xl font-black tracking-tighter uppercase leading-none">
              MAULA<span className="text-purple-500">.AI</span>
            </span>
        ...
        <nav className="flex-1 space-y-2 relative z-10">
          <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] mb-4 ml-6">
            Operation
          </div>
          <SidebarItem
            id="overview"
        ...
        <div className="mt-auto pt-8 border-t border-white/5 space-y-4 relative z-10">
          <div className="flex items-center gap-4 p-4 glass rounded-2xl border border-white/5 group cursor-pointer hover:bg-white/5 transition-colors">
            <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-purple-500 to-pink-500 flex items-center justify-center font-black shadow-lg">
              AD
            </div>
            <div className="flex-1 overflow-hidden">
              <div className="text-xs font-black truncate uppercase tracking-widest group-hover:text-purple-400 transition-colors">
                Admin_882
              </div>
          ...
          <button
            onClick={() => setView('home')}
            className="w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-red-500/40 hover:text-red-500 hover:bg-red-500/5 transition-all"
          >
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden bg-[radial-gradient(circle_at_50%_-20%,_#0d0d1a_0%,_#02000a_100%)]">
        ...
        <header className="h-24 border-b border-white/5 px-12 flex items-center justify-between glass backdrop-blur-md z-40 relative">
          ...
          <div className="flex items-center gap-6">
            ...
            <button
              onClick={() => setView('home')}
              className="px-6 py-2 border border-white/10 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-white/5 hover:border-white/20 transition-all"
            >
              Exit HUD
            </button>
          </div>
        ...
        <div ref={contentRef} className="flex-1 overflow-y-auto p-12 scrollbar-hide relative z-10">
          {activeTab === 'overview' && (
            <OverviewTab toggleTab={setActiveTab} fleetCount={purchasedIds.length} />
          )}
          {activeTab === 'marketplace' && (
            <ModuleGrid
              type="available"
              purchasedIds={purchasedIds}
              setPurchased={setPurchasedIds}
              query={searchQuery}
            />
          )}
          {activeTab === 'fleet' && (
  ...
  <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="glass p-10 rounded-[3rem] border border-white/5 group hover:bg-white/[0.02] transition-colors relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-10 relative z-10">
          <h3 className="text-xs font-black uppercase tracking-[0.3em] text-white/40">
            Neural Traffic Density
          </h3>
          <button
            onClick={() => toggleTab('analytics')}
            className="text-[9px] font-black uppercase tracking-widest text-purple-400 flex items-center gap-1"
          >
        ...
        <div className="h-64 flex items-end gap-3 relative z-10">
          {[40, 70, 45, 90, 65, 80, 50, 95, 40, 60, 85, 30, 70, 55, 90].map((h, i) => (
            <div
              key={i}
              className="flex-1 bg-white/5 rounded-t-lg relative overflow-hidden group/bar"
            >
              <div
      ...
      <div className="glass p-10 rounded-[3rem] border border-white/5">
        <h3 className="text-xs font-black uppercase tracking-[0.3em] text-white/40 mb-10">
        ...
        <div className="space-y-4">
          {[
            {
              node: 'FraudGuard',
              msg: 'Botnet attempt blocked at North PoP',
              level: 'Critical',
              time: '2m ago',
            },
            {
              node: 'NetDefender',
              msg: 'Anomalous DNS tunnel detected',
              level: 'Warning',
              time: '14m ago',
            },
            {
            ...
            {
              node: 'SIEMCommander',
              msg: 'Lattice baseline recalibrated',
              level: 'Success',
              time: '3h ago',
            },
          ].map((log, i) => (
            <div
              key={i}
              className="flex items-center gap-6 p-5 rounded-3xl bg-white/[0.02] border border-white/5 hover:border-white/20 transition-all cursor-pointer group"
            >
              <div
                className={`w-2.5 h-2.5 rounded-full ${log.level === 'Critical' ? 'bg-red-500 shadow-[0_0_15px_rgba(239,68,68,0.5)]' : log.level === 'Warning' ? 'bg-amber-500' : log.level === 'Success' ? 'bg-emerald-500' : 'bg-blue-500'}`}
              />
              <div className="flex-1">
                <div className="text-[11px] font-bold text-white/80 group-hover:text-white transition-colors">
                  {log.msg}
                </div>
                <div className="text-[8px] uppercase font-black text-white/20 tracking-widest mt-1">
                  {log.node} • {log.time}
                </div>
              </div>
        ...
        <button
          ...
          className="w-full mt-8 py-4 border border-white/5 rounded-2xl text-[9px] font-black uppercase tracking-[0.3em] text-white/30 hover:bg-white/5 hover:text-white transition-all"
        >
          Enter Log Terminal
        </button>
      </div>
...
const StatCard = ({ title, value, sub, icon, color }: any) => (
  <div className="glass p-8 rounded-[2.5rem] border border-white/5 flex flex-col justify-between group hover:border-white/20 transition-all">
    ...
    <div>
      ...
      <div
        className={`text-[8px] font-bold text-${color} mt-3 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity`}
      >
        {sub}
      </div>
    </div>
...
const ModuleGrid = ({
  ...
  const filtered = tools.filter((t) => {
    ...
    const matchesType =
      type === 'purchased' ? purchasedIds.includes(t.id) : !purchasedIds.includes(t.id);
      ...
      <div className="flex justify-between items-end border-b border-white/5 pb-8">
        ...
        {type === 'available' && (
          <div className="flex gap-4">
            <span className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[9px] font-black uppercase tracking-widest cursor-pointer hover:bg-white/10">
              Sort by Risk
            </span>
            <span className="px-4 py-2 bg-purple-500/10 border border-purple-500/30 rounded-xl text-[9px] font-black uppercase tracking-widest cursor-pointer hover:bg-purple-500/20 text-purple-400">
              Featured
            </span>
          </div>
        )}
      ...
      {filtered.length === 0 ? (
        <div className="h-96 flex flex-col items-center justify-center text-center space-y-6 glass rounded-[4rem] border border-white/5 border-dashed">
          ...
          <div className="space-y-2">
            ...
            <p className="text-white/30 text-xs max-w-xs">
              Adjust your lattice search parameters or visit the marketplace to acquire new nodes.
            </p>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filtered.map((tool) => (
            <div
              key={tool.id}
              className={`glass p-8 rounded-[2.5rem] border transition-all group relative overflow-hidden flex flex-col h-full ${type === 'purchased' ? 'border-purple-500/20 shadow-2xl shadow-purple-950/20' : 'border-white/5 hover:border-white/20'}`}
            >
              <div className="flex justify-between items-start mb-8">
                <div
                  className={`p-4 rounded-2xl bg-${tool.theme.primary}/10 text-${tool.theme.primary} group-hover:scale-110 transition-transform duration-500`}
                >
                  <Shield className="w-6 h-6" />
                </div>
                {type === 'purchased' ? (
                  ...
                  <div className="text-lg font-black tracking-tighter text-white/40 font-mono">
                    $249/mo
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 space-y-3 mb-10">
                <h3 className="text-xl font-black uppercase tracking-tight group-hover:text-purple-400 transition-colors leading-none">
                  {tool.name}
                </h3>
                ...
                <div className="flex flex-wrap gap-2 pt-4">
                  <span className="px-2 py-1 bg-white/5 rounded-md text-[8px] font-black uppercase tracking-widest text-white/40">
                    {tool.category}
                  </span>
                  <span className="px-2 py-1 bg-white/5 rounded-md text-[8px] font-black uppercase tracking-widest text-white/40">
                    {tool.stats.accuracy} Acc
                  </span>
                </div>
              </div>
              ...
              <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                <div className="text-[8px] font-mono text-white/10 uppercase leading-none">
                  Node_ID: 0x{tool.id.toString(16).toUpperCase()}
                </div>
              </div>
            </div>
          ))}
    ...
    <div className="glass p-20 rounded-[4rem] border border-white/5 text-center space-y-12 relative overflow-hidden min-h-[600px] flex flex-col justify-center">
      ...
      <div className="space-y-6 relative z-10">
        ...
        <p className="text-white/40 max-w-lg mx-auto text-lg leading-relaxed font-medium">
          The Maula Deep-Learning engine is synthesizing terabytes of organizational telemetry.
          Global interception heatmaps will manifest upon 24-hour baseline verification.
        </p>
      </div>
      <div className="flex gap-4 justify-center relative z-10">
        <div className="px-6 py-3 bg-white/5 rounded-full text-[10px] font-black uppercase tracking-widest text-white/20 border border-white/10 flex items-center gap-2">
          <RefreshCw className="w-3 h-3 animate-spin" /> Ingesting Data
        </div>
        <div className="px-6 py-3 bg-white/5 rounded-full text-[10px] font-black uppercase tracking-widest text-white/20 border border-white/10 flex items-center gap-2">
          <RefreshCw className="w-3 h-3 animate-spin" /> Training Model_v4
        </div>
      </div>
    </div>
  </div>
...
const LogsTab = () => (
  <div className="space-y-8 animate-in fade-in duration-500">
    <div className="flex justify-between items-center px-4">
      <div className="flex gap-4">
        <span className="px-4 py-1.5 bg-purple-500/20 border border-purple-500/40 rounded-full text-[9px] font-black uppercase tracking-widest text-purple-400">
          All Logs
        </span>
        <span className="px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-[9px] font-black uppercase tracking-widest text-white/20 hover:text-white transition-colors cursor-pointer">
          Critical Only
        </span>
      </div>
    ...
    <div className="glass p-10 rounded-[3rem] border border-white/5 bg-black/60 font-mono text-[11px] leading-relaxed max-h-[70vh] overflow-y-auto scrollbar-hide shadow-inner border-t border-l border-white/10">
      {[...Array(40)].map((_, i) => (
        <div
          key={i}
          className="flex gap-6 py-2 border-b border-white/[0.02] last:border-0 opacity-50 hover:opacity-100 transition-opacity group"
        >
          <span className="text-white/20 shrink-0 font-bold">
            [{new Date().toISOString().split('T')[1].split('Z')[0]}]
          </span>
          <span className="text-purple-400 font-black shrink-0 uppercase tracking-tighter">
            LATTICE_DAEMON
          </span>
          <span className="text-white/60 group-hover:text-white transition-colors">
            Module_0x{Math.floor(Math.random() * 50).toString(16)} status: OK. Pulse check
            completed. Verified secure.
          </span>
          <span className="ml-auto text-emerald-500 font-bold tracking-widest text-[9px] uppercase">
            SUCCESS
          </span>
        </div>
      ))}
      <div className="py-4 text-purple-500/40 animate-pulse italic font-black uppercase tracking-[0.4em] text-center pt-10">
        --- Waiting for Incoming Transmission Streams ---
      </div>
    </div>
  </div>
);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div className="glass p-12 rounded-[4rem] border border-emerald-500/20 relative overflow-hidden group">
        ...
        <div className="flex justify-between items-center mb-10 relative z-10">
          ...
          <span className="px-4 py-2 rounded-full bg-emerald-500 text-black text-[9px] font-black uppercase tracking-widest shadow-[0_0_20px_rgba(16,185,129,0.4)]">
            Verified Active
          </span>
        </div>
        ...
        <p className="text-white/40 text-xs font-medium relative z-10 leading-relaxed">
          Enterprise plan subscription is active. Next cycle begins February 1, 2026. Billed to
          primary corporate terminal account.
        </p>
        <button className="mt-10 px-8 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
          Download Forecast
        </button>
      </div>
      ...
      <div className="space-y-6">
        ...
        {[
          ...
          <div
            key={i}
            className="flex items-center justify-between p-8 glass rounded-[2.5rem] border border-white/5 hover:bg-white/5 transition-all cursor-pointer group"
          >
            <div className="flex items-center gap-6">
              <div className="p-3 bg-white/5 rounded-2xl group-hover:bg-purple-500/20 group-hover:text-purple-400 transition-all">
                <CreditCard className="w-6 h-6" />
              </div>
              <div>
                <div className="text-sm font-black tracking-tight">{inv.id}</div>
                <div className="text-[9px] font-black text-white/20 uppercase tracking-widest mt-1">
                  {inv.date}
                </div>
              </div>
            </div>
            <div className="text-right">
              ...
              <div className="text-[9px] font-black text-emerald-400 uppercase tracking-widest mt-1">
                {inv.status}
              </div>
            </div>
          </div>
        ))}
        <button className="w-full py-4 text-[9px] font-black uppercase tracking-[0.4em] text-white/20 hover:text-white transition-colors">
          View All Invoices
        </button>
      </div>
    </div>
    ...
    <div className="max-w-5xl space-y-12 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        <div className="space-y-10">
          <h3 className="text-xs font-black uppercase tracking-[0.4em] text-white/30 px-2 italic">
            Lattice Global Security
          </h3>
          <div className="space-y-4">
            <SettingsRow
              label="Neural Auto-Heal"
              desc="Allow lattice to autonomously remediate drift"
              active
            />
            <SettingsRow
              label="FIPS Encryption"
              desc="Enforce hardware-level encryption across nodes"
              active
            />
            <SettingsRow
              label="Biometric Access"
              desc="Require neural/biometric ID for node changes"
            />
            <SettingsRow
              label="Quantum Shield"
              desc="Activate post-quantum cryptographic wrappers"
              active
            />
          </div>
        </div>
        <div className="space-y-10">
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            >
              <div className="flex items-center gap-6">
                <div className="p-4 bg-white/5 rounded-2xl text-white/20 group-hover:text-purple-400 transition-colors">
                  <Key className="w-6 h-6" />
                </div>
                <div>
                  <div className="text-sm font-black uppercase tracking-tight">API Access Keys</div>
                  <div className="text-[10px] text-white/30 font-medium">
                    Manage cross-origin node integrations
                  </div>
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-6">
                <div className="p-4 bg-white/5 rounded-2xl text-white/20 group-hover:text-emerald-400 transition-colors">
                  <User className="w-6 h-6" />
                </div>
                <div>
                  <div className="text-sm font-black uppercase tracking-tight">Admin Hierarchy</div>
                  <div className="text-[10px] text-white/30 font-medium">
                    Define node-level operator permissions
                  </div>
                </div>
              </div>
              <ArrowUpRight className="w-5 h-5 text-white/10 group-hover:text-white transition-all" />
            ...
            >
              <div className="flex items-center gap-6">
                <div className="p-4 bg-white/5 rounded-2xl text-white/20 group-hover:text-amber-400 transition-colors">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-white/30 font-medium">
                    Configure geographic data residency perms
                  </div>
                </div>
              </div>
              <ArrowUpRight className="w-5 h-5 text-white/10 group-hover:text-white transition-all" />
...
const SettingsRow = ({
  ...
  <div className="flex items-center justify-between p-8 glass rounded-[2.5rem] border border-white/5 hover:bg-white/5 transition-all">
    ...
    <div
      className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors ${active ? 'bg-purple-600' : 'bg-white/10'}`}
    >
      <div
        className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all shadow-md ${active ? 'left-7' : 'left-1 opacity-40'}`}
      />
    </div>
  </div>
);
