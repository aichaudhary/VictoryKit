...
const Header: React.FC = () => {
  const { setView, view } = useScroll();

  if (view.startsWith('dashboard')) return null; // Dashboard has its own header/nav
    ...
    <header className="fixed top-0 left-0 w-full z-[100] glass px-6 md:px-12 py-4 flex items-center justify-between transition-all duration-300">
      ...
      <div className="flex items-center gap-4">
        <button 
          ...
          className="hidden sm:flex items-center gap-2 text-sm font-medium text-white hover:text-purple-400 transition-colors"
        >
          <LogIn className="w-4 h-4" /> Login
        </button>
        <button 
          onClick={() => setView('dashboard')}
          className="bg-white text-black text-sm font-bold px-5 py-2 rounded-full flex items-center gap-2 hover:bg-purple-50 transition-all shadow-xl shadow-white/5"
        >
